...
(jezeli mamy dwa przedzialy [a, b] oraz [a, c] i b > c to po posortowaniu wczesniej bedzie przedzial [a, b], wiec
...
koniec i szukamy binarnie pierwszego przedzialu o silnie wiekszym koncu przedialu od naszego obecnego przedzialu z
listy L używajac funkcji binary_search() i obliczamy wspolczynnik current_depth jako:
...
emementow pozostalych do konca listy jest mniej niz max_depth lub gdy docieramy do konca list (gdy max_depth == 0).

Uzasadnienie poprawnosci algorytmu:
Lemat 1: Przedzial p o max detph nie jest zawarty w zadnym z przedzialow.
    Dowod nie w prost: Jesli istnieje przedzial k, ktory zawiera p, to zawiera on wyszstkie przedzialy,
    ktore zawiera p, wiec zawiera on max depth + 1 przedzialow, co jest sprzeczne z zalozeniem, ze p ma max depth.
...
2 razy sortujemy liste o dlugosci n, a nastepnie liniowo przechodzimy przez liste o dlugosci n (n) i szukamy biarnie
w drugiej liscie o dlugosci n (logn), wiec mamy: 2*O(nlogn) + O(nlogn) = O(nlogn)
...
def partition_by_ending(sorted_by_end_list, p, r):
    ...
    sorted_by_end_list[i +
                       1], sorted_by_end_list[r] = sorted_by_end_list[r], sorted_by_end_list[i+1]
    return i + 1
...
def depth(L):
    ...
    for i in range(n):
        ...
        if sorted_by_end_list[n-1][1] == end:
            ...
            current_depth = n - 1 - i - \
                (n - binary_search(sorted_by_end_list, end, 0, n-1))
